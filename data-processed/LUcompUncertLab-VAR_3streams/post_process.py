...
if __name__ == "__main__":
    ...
    forecast["forecast_date"] = pd.to_datetime(io.forecast_date) + datetime.timedelta(
        days=1
    )  # THIS ASSUMES WE RUN ON SUNDAY
    ...
    forecast = forecast.loc[
        ~forecast.target.str.contains("^0 day")
    ]  # REMOVE SUNDAY "DAY" FORECASTS

    day = io.getForecastDate()
    forecast.to_csv(
        "./location_specific_forecasts/{:s}_LUcompUncertLab-VAR4Streams__{:s}.csv.gz".format(
            day, io.fmtlocation
        ),
        header=True,
        index=False,
