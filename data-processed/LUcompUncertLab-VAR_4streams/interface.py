...
class interface(object):
    ...
    def buildDataForModel(self):
        import numpy as np
        if len(str(self.location)) > 2:
            y = np.array(self.county_data[['date','county_cases','state_deaths','state_hosps','county_vac_count']].set_index("date"))
        else:
            y = np.array(self.data[['date','cases','deaths','hosps','vac_count']].set_index("date"))    
